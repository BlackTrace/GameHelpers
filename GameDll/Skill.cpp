...
#include "baseaddr.h"
#include "GameCall.h"
Skill::Skill(DWORD dwNodeBase, DWORD _index) : m_dwNodeBase(dwNodeBase), m_index(_index) , m_Distance(0.0f), m_skWidth(0.0f)
{
	...
	if (temp)
	{
		...
		if (temp2)
		{
			...
			memcpy(m_skDistance3, (void*)(temp2 + Base_SkillOffset_Range + 0x1c + 0x1c), 0x1c);
			//读取技能攻击力
...
float Skill::GetDistance3() 
{
	auto level = GetLevel();
	if (level < 7)
	{
		return  m_skDistance3[level];
	}
...
WORD Skill::GetSkillType()
{
	auto temp = utils::read<DWORD>(m_dwNodeBase + 0xf8);
	if (temp)
	{
		auto temp1 = utils::read<DWORD>(temp + 0x70);
		if (temp1)
		{
			return utils::read<WORD>(temp1 + 0x8);
		}
	}
	return 0;
}
...
{
	return  utils::read<float>(m_dwNodeBase + 0xcc);
}
